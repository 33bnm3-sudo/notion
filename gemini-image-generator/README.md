...
예: 릴스에서 본 티셔츠 사진을 참고 이미지로 넣고 "그 로고만 분리해서, 검정 실루엣, 배경 없이 그려줘" 같은 프롬프트로 이미지를 만들고 → `logo-bg-remover`로 배경 제거 → `logo-to-svg`로 벡터화 → 3D 모델링. 이 모듈은 그 파이프라인의 첫 단계입니다.

참고 이미지(0장 이상)를 텍스트 프롬프트와 함께 보낼 수 있어서, 텍스트만으로 새로 그리는 것뿐 아니라 "이 사진 보고 이렇게 바꿔줘" 같은 이미지 편집/참조 생성도 됩니다 — Gemini 앱/AI Studio 웹에서 이미지 올리고 프롬프트 넣는 것과 같은 방식입니다.
...
이 샌드박스에는 실제 Gemini API 키가 없어서 진짜 API 호출까지는 못 해봤습니다. 대신 공식 문서에 나온 응답 스키마(`candidates[].content.parts[].inlineData.data`, `promptFeedback.blockReason`, `error.message` 등)를 그대로 흉내 낸 가짜 JSON으로 파싱 로직만 단위 테스트했고 (`cargo test`, 5개 통과), HTTP 통신 부분은 컴파일 확인만 했습니다. 실제 키로 한 번 테스트해보시는 걸 권장합니다.
...
// 참고 이미지(예: 티셔츠 사진)를 같이 넣어서 편집/추출
gemini_image_generator::generate_image(
    "YOUR_API_KEY",
    "이 사진 속 로고만 분리해서 검정 실루엣으로, 배경 없이 그려줘",
    &[Path::new("tshirt-photo.jpg")],
    &opts,
    Path::new("extracted-logo.png"),
)?;
```

참고 이미지 확장자는 `png` / `jpg` / `jpeg` / `webp`만 지원합니다 (`guess_mime_type`).
...
async fn generate_logo_image(
    ...
    let opts = gemini_image_generator::ImageOptions::default();
    let reference_path = reference_image.as_deref().map(std::path::Path::new);
    let references: &[&std::path::Path] = match &reference_path {
